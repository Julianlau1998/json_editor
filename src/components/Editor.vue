            ...
            <transition name="fade" mode="out-in">
                <span :key="1">
                  <label class="label is-pointer noselect button is-primary">
                    <input @change="fileInput" accept=".txt, .js, .json" type="file" required/>
                    <span>
...
export default {
    ...
    methods: {
        ...
        createFileLink (fileName) {
            const file = new Blob([this.code], { type: "data:text/csv;charset=utf-8" }, `${fileName}.json`)
        ...
        highlighter(code) {
          if (Prism.languages.json) {
            this.json = Prism.languages.json
          }
          return highlight(code, this.json, '')
        },
    }
}
</script>
